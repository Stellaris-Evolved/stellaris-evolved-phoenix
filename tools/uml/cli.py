...
from tools.uml.utils import load_config, Writer, BASE_PATH, create_sprites_for_cleanup, AUTH_SWAPS_INLINE_NAME, \
    ...
    TRAIT_INLINE_NAME_TEMPLATE_SUFFIX, TRAIT_INLINE_NAME_TEMPLATE, HOARDER_BUTTONS_TEMPLATE, TENET_BUTTONS_TEMPLATE
...
@click.option('--prefix', default='tec')
...
def generate_tags(config: str, local_config: str, base_mod_path: str):
    ...
    with open(base_mod_path / "common/scripted_modifiers/zz_tec_autogenerated_tags_scripted_modifiers.txt", 'w') as f:
...
def generate_hoarder_autogen_files(config: str, local_config: str, base_mod_path: str):
    ...
    with open(base_mod_path / "interface/zzz_tec_autogenerated_hoarder_buttons.gui", 'w') as f:
        f.write('guiTypes = {\n')
        f.write("""
...
@click.option('--local-config', default=Path(BASE_PATH) / 'local_config.yml')
@click.option('--base-mod-path', default=Path(BASE_PATH) / '../..')
def generate_tenet_autogen_files(config: str, local_config: str, base_mod_path: str):
    base_mod_path = Path(base_mod_path).resolve()
    cfg = load_config(config)
    local_cfg = load_config(local_config)
    cfg |= local_cfg
    ...
    with open(str(base_mod_path / f"common/inline_scripts/cui/tenet/tec_tenet_declarations.txt")) as f:
        tenet = None
        modifiers = None
        icon = None
        x, y = None, None
        ...
        for line in f:
            if should_look_for_modifier_end:
                ...
                if matches and matches.group(1):
                    if tenet:
                        tenets.append([tenet, icon, modifiers, [x, y]])
                    tenet = matches.group(1)
                    icon = None
                    modifiers = None
                    x, y = None, None
    ...
    new_line = '\n'
    with open(base_mod_path / "common/static_modifiers/z_tec_autogenerated_tenet_static_modifiers.txt", 'w') as f:
        for tenet, icon, modifiers, _ in tenets:
            if modifiers:
                f.write(textwrap.dedent(f"""\
                    {tenet}_modifier = {{
    ...
    with open(base_mod_path / "common/scripted_loc/z_tec_autogenerated_tenet_scripted_loc.txt", 'w') as f:
        for tenet, icon, modifiers, _ in tenets:
            f.write(textwrap.dedent(f"""\
                defined_text = {{
                    name = {tenet}_cost
                    value = value:tec_tenet_cost|TENET|{tenet}|
                }}
            """))

    with open(base_mod_path / "interface/z_tec_autogenerated_tenet_sprites.gfx", 'w') as f:
        f.write('spriteTypes = {\n')
        for tenet, icon, modifiers, _ in tenets:
            if icon:
                f.write(textwrap.dedent(f"""\
                ...
                f.write(textwrap.dedent(f"""\
                    spriteType = {{
                        name = GFX_{tenet}
                        effectFile = gfx/FX/buttonstate_onlydisable.shader
                        textureFile = gfx/interface/icons/tec_tenets/{tenet}.dds
                    }}
                """))
        f.write('\n}')

    tenet_template = ""
    with open(base_mod_path / TENET_BUTTONS_TEMPLATE) as f:
        tenet_template = f.read().rstrip()

    with open(base_mod_path / "interface/z_tec_autogenerated_tenet_buttons.gui", 'w') as f:
        f.write(textwrap.dedent(f"""\
            @tec_button_size = 45
        ...
        """))
        f.write('guiTypes = {\n')
        for tenet, icon, modifiers, (x, y) in tenets:
            if x is not None and y is not None:
                f.write(
                    tenet_template.replace('$name$', tenet).replace('$x$', x).replace('$y$', y)
                )
        f.write('\n}')


    with Writer(base_mod_path / "localisation/english/replace/z_tec_autogenerated_tenet_l_english.yml") as writer:
        writer.write_language('english')

        for tenet, _, _, _ in tenets:
            writer.write_localization(f"tec_has_{tenet}", f"$tec_has_empire_tenet$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"tec_has_{tenet}_yes", f"£trigger_yes£ $tec_has_empire_tenet$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"tec_has_{tenet}_no", f"£trigger_no£ $tec_has_empire_tenet$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"tec_activate_tenet_{tenet}", f"$tec_activate_tenet_effect$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"tec_deactivate_tenet_{tenet}", f"$tec_deactivate_tenet_effect$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"tec_has_last_activated_{tenet}", f"$tec_has_empire_last_activated_tenet$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"tec_has_last_activated_{tenet}_yes", f"£trigger_yes£ $tec_has_empire_last_activated_tenet$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"tec_has_last_activated_{tenet}_no", f"£trigger_no£ $tec_has_empire_last_activated_tenet$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"tec_has_last_deactivated_{tenet}", f"$tec_has_empire_last_deactivated_tenet$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"tec_has_last_deactivated_{tenet}_yes", f"£trigger_yes£ $tec_has_empire_last_deactivated_tenet$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"tec_has_last_deactivated_{tenet}_no", f"£trigger_no£ $tec_has_empire_last_deactivated_tenet$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"tec_has_tenet_points_available_{tenet}", f"$tec_has_tenet_points_available$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"tec_has_tenet_points_available_{tenet}_yes", f"£trigger_yes£ $tec_has_tenet_points_available$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"tec_has_tenet_points_available_{tenet}_no", f"£trigger_no£ $tec_has_tenet_points_available$ ['tradition:{tenet}', ${tenet}$]")
            writer.write_localization(f"{tenet}_modifier", f"${tenet}$")
            writer.write_localization(f"{tenet}_header", f"§H${tenet}$§!")
            writer.write_localization(f"{tenet}_key", f"{tenet}")
            writer.write_localization(f"{tenet}_cost", f"§Y[FROM.{tenet}_cost]§!")
            writer.write_localization(f"{tenet}_cost_red", f"§R[FROM.{tenet}_cost]§!")
            writer.write_localization(f"{tenet}_cost_header", f"Cost: ${tenet}_cost$")
            writer.write_localization(f"faction_demand_{tenet}", f"${tenet}$")
            writer.write_localization(f"faction_demand_{tenet}_liked_desc", f"Having the §Y${tenet}$§! Tenet pleases the [Root.GetName]")
            writer.write_localization(f"faction_demand_{tenet}_disliked_desc", f"Having the §Y${tenet}$§! Tenet displeases the [Root.GetName]")
